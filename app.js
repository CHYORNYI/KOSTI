...
function save(){try{localStorage.setItem("kostiV6",JSON.stringify({...state,selected:[...state.selected]}))}catch(e){}}
function load(){try{const s=JSON.parse(localStorage.getItem("kostiV6"));if(s&&!s.over)state={...state,...s,selected:new Set(s.selected||[])}}catch(e){}}
...
function animateDice(){
  ...
  setTimeout(()=>[...diceEl.children].forEach(d=>d.classList.remove("rolling")),900);
}
function playerRoll(){
  if(state.rolled){
    const s=score(selectedVals());
...
function newGame(){
  ...
  try{localStorage.removeItem("kostiV6")}catch(e){}
  statusEl.textContent="Ваш ход. Бросьте кости.";
  render();
}
