...
function save(){try{localStorage.setItem("kostiV7",JSON.stringify({...state,selected:[...state.selected]}))}catch(e){}}
function load(){try{const s=JSON.parse(localStorage.getItem("kostiV7"));if(s&&!s.over)state={...state,...s,selected:new Set(s.selected||[])}}catch(e){}}
...
function score(vals){
  ...
  if(k==="123456")return 1500;
  if(k==="12345")return 500;
  if(k==="23456")return 750;
  ...
  for(let v=1;v<=6;v++){
    const n=c[v];
    if(n>=3){
      s+=(v===1?1000:v*100)*Math.pow(2,n-3);
      c[v]=0;
    }
  }
...
function toggle(i){
  ...
  const s=score(selectedVals());
  statusEl.textContent=s?`Отобрано: ${s} очков`:"Эта комбинация очков не даёт";
  beep(250,.035);
  render();
}
function animateDice(){
  [...diceEl.children].forEach(d=>d.classList.add("rolling"));
  setTimeout(()=>[...diceEl.children].forEach(d=>d.classList.remove("rolling")),1100);
}
function playerRoll(){
  if(state.rolled){
    const s=score(selectedVals());
    if(!s)return;
    state.turn+=s;
    state.available-=state.selected.size;
    if(state.available<=0)state.available=6;
  }
  state.dice=Array.from({length:state.available},rand);
  state.selected.clear();
  state.rolled=true;
  beep(120,.12);
  render();
  animateDice();
  if(!hasScore(state.dice)){
    state.turn=0;
    state.rolled=false;
    statusEl.textContent="Неудачный бросок. Очки раунда сгорели.";
    render();
    setTimeout(startAI,1200);
  }else{
    statusEl.textContent="Выберите кости с очками.";
  }
}
function bank(){
  const s=score(selectedVals());
  ...
  statusEl.textContent="Очки записаны. Ход соперника.";
  render();
  setTimeout(startAI,1100);
}
function win(w){
  if(state[w]>=target){
    state.over=true;
    statusEl.textContent=w==="player"?"Вы выиграли партию!":"Соперник победил.";
    render();
    return true;
  }
  return false;
...
function best(vals){
  ...
  for(let m=1;m<(1<<vals.length);m++){
    const idx=[],a=[];
    for(let i=0;i<vals.length;i++)if(m>>i&1){idx.push(i);a.push(vals[i])}
    const s=score(a);
    if(s>b.score||(s===b.score&&idx.length<b.idx.length))b={score:s,idx};
  }
  return b;
}
function startAI(){
...
function aiRoll(){
  ...
  animateDice();
  setTimeout(()=>{
    const b=best(state.dice);
    if(!b.score){
      state.aiTurn=0;
      statusEl.textContent="Сопернику не повезло. Очки сгорели.";
      render();
      setTimeout(endAI,1100);
      return;
    }
    b.idx.forEach(i=>state.selected.add(i));
    state.aiTurn+=b.score;
    state.available-=b.idx.length;
    if(state.available<=0)state.available=6;
    render();
    const need=target-state.ai;
    const bankNow=state.aiTurn>=need||state.aiTurn>=(state.available<=2?650:900)||(state.aiTurn>=550&&Math.random()<.55);
    if(bankNow){
      state.ai+=state.aiTurn;
      statusEl.textContent=`Соперник забирает ${state.aiTurn} очков.`;
      state.aiTurn=0;
      render();
      if(win("ai"))return;
      setTimeout(endAI,1100);
    }else{
      statusEl.textContent=`Соперник рискует. Раунд: ${state.aiTurn}.`;
      render();
      setTimeout(aiRoll,1200);
    }
  },1000);
}
function endAI(){
...
function newGame(){
  state={player:0,ai:0,turn:0,whose:"player",dice:[],available:6,selected:new Set(),rolled:false,sound:state.sound,over:false,aiTurn:0};
  try{localStorage.removeItem("kostiV7")}catch(e){}
  statusEl.textContent="Ваш ход. Бросьте кости.";
  render();
}
...
hideRules.onclick=()=>document.querySelector(".playfield").classList.add("rules-hidden");
newBtn.onclick=()=>{if(confirm("Начать новую игру?"))newGame()};
